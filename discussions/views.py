...
def discussion_list(request):
    ...
    discussions = (
        Discussion.objects.select_related("author", "topic")
        .annotate(reply_count=Count("replies"), vote_score=Count("votes__value"))
        .order_by("-created_at")
    )
...
def vote_discussion(request, pk):
    if request.method == "POST":
        ...
        vote, created = Vote.objects.get_or_create(
            user=request.user, discussion=discussion, defaults={"value": value}
        )
        if not created:
            if vote.value == value:
                vote.delete()
                ...
                vote.value = value
                vote.save()
        vote_count = discussion.votes.aggregate(score=Count("value"))["score"]
...
def discussion_delete(request, pk):
    ...
    return render(
        request,
        "discussions/discussion_confirm_delete.html",
        {"discussion": discussion},
    )
