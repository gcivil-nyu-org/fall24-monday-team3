...
class RoommatePostModelTest(TestCase):
    ...
    def test_comment_str(self):
        comment = Comment.objects.create(
            post=self.roommate_post, user=self.user, content="Test comment"
        )
        expected = f"Comment by {self.user} on {self.roommate_post.name}"
        self.assertEqual(str(comment), expected)

    def test_favorite_str(self):
        favorite = Favorite.objects.create(user=self.user, post=self.roommate_post)
    ...
    def test_comment_is_reply(self):
        parent_comment = Comment.objects.create(
            post=self.roommate_post, user=self.user, content="Parent comment"
        )
